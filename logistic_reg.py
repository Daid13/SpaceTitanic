...
main = pd.concat(
    [
        ...
        pd.DataFrame(
            {
                "HomePlanet_Earth": main["HomePlanet"] == "Earth",
                "HomePlanet_Europa": main["HomePlanet"] == "Europa",
            }
        ),
    ],
    axis=1,
...
main = pd.concat(
    [
        ...
        pd.DataFrame(
            {
                "Destination_TRAPPIST": main["Destination"] == "TRAPPIST-1e",
                "Destination_PSO": main["Destination"] == "PSO J318.5-22",
            }
        ),
    ],
    axis=1,
...
test = pd.concat(
    [
        ...
        pd.DataFrame(
            {
                "HomePlanet_Earth": test["HomePlanet"] == "Earth",
                "HomePlanet_Europa": test["HomePlanet"] == "Europa",
            }
        ),
    ],
    axis=1,
...
test = pd.concat(
    [
        ...
        pd.DataFrame(
            {
                "Destination_TRAPPIST": test["Destination"] == "TRAPPIST-1e",
                "Destination_PSO": test["Destination"] == "PSO J318.5-22",
            }
        ),
    ],
    axis=1,
...
# ---- 3. TRAIN-TEST SPLIT
main_train = main.sample(frac=0.8, random_state=400)
...
train_results = pd.DataFrame(
    {"actual": main_test["Transported"], "predicted": train_predicts}
)
train_results["predicted"] = (train_results["predicted"] >= 0.5).astype("int32")
...
submission = pd.DataFrame(
    {"PassengerId": test["PassengerId"], "Transported": (test_predicts >= 0.5)}
)
