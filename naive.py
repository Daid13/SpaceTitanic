...
def categoricalise(dataframe):
    # print(dataframe)
    dataframe["Cabin1"] = dataframe.apply(
        lambda row: str(row.Cabin).split("/")[0], axis=1
    )
    # print(dataframe)
    # dataframe["Cabin2"]=dataframe.apply(lambda row: str(row.Cabin).split('/')[1],axis=1)
    dataframe["Cabin3"] = dataframe.apply(
        lambda row: str(row.Cabin).split("/")[2], axis=1
    )
    ...
    dataframe["ShoppingMall1"] = dataframe.apply(
        lambda row: row.ShoppingMall > 0, axis=1
    )
